...
#eksplorasi data uji
plot(test_data.ts, col="green",main="Plot data uji")
points(test_data.ts)
```
...
ggplot() + 
  ...
  labs(x = "Periode Waktu", y = "Harga Penutup", color = "Legend") +
...
Pola data yang dihasilkan dari set data Saham Antam Harian adalah konstan.

Metode pemulusan SMA cocok digunakan untuk pola data stasioner atau konstan.Set data saham ANTAM yang saya gunakan juga merupakan daya yang konstan. Prinsip dasar metode pemulusan ini adalah data pemulusan pada periode ke-t merupakan rata rata dari m buah data pada periode ke-t hingga periode ke (t-m+1). Data pemulusan pada periode ke-t selanjutnya digunakan sebagai nilai peramalan pada periode ke t+1
...
```{r}
ts.plot(data.ts, xlab="Periode Waktu ", ylab="Harga Penutupuan", main= "SMA N=2 Data Closing Price")
points(data.ts)
...
Dalam hal ini nilai MAPE data latih pada metode pemulusan SMA kurang dari 6%, nilai ini dapat dikategorikan sebagai nilai akurasi yang sangat baik. Selanjutnya dilakukan perhitungan nilai MAPE data uji pada metde pemulusan SMA.
...
DPerhitungan akurasi menggunakan data latih menghasilkan nilai MAPE yang kurang dari 10% sehingga nilai akurasi ini dapat dikategorikan sebagai sangat baik.
...
```
Kesimpulan:
mape sma (training) : 5.521
mape sma (uji) : 8.532

mape ses (training) : 7.262 (0.2)
mape ses (training) : 5.35 (0.7)

mape ses (uji) : 6.2750 (0.2)
mape ses (uji) : 4.5670 (0.7)

Untuk data training (latih) metode yang lebih baik digunakan adalah  metode SMA sedangkan untuk data uji metode yang lebih baik adalah SES dengan nilai MAPE yang lebih kecil.
